...
fun main() {
    ...
    val totalElecPriceOver500 = products
        ...
        .filter { it.price > 500 }.sumOf { it.price }

    // ผลลัพธ์ที่ได้จะเป็น Map<String, List<Product>>
    val productsByPriceRange = products.groupBy { product ->
        when {
            product.price <= 1000 -> "ราคาไม่เกิน 1,000 บาท"
            product.price >= 10000 -> "ราคา 10,000 บาทขึ้นไป"
            else -> "ราคา 1,001 - 9,999 บาท"
        }

    }

    productsByPriceRange.forEach { (range, productList) ->
        // พิมพ์ชื่อกลุ่ม (Key ของ Map)
        println("\n--- กลุ่มสินค้า: $range ---")

        // วนลูปในรายการสินค้าของกลุ่มนั้นๆ (Value ของ Map)
